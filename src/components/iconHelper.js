...
function getFavIconFirefox(urlString) {
  ...
  // if(baseurl && baseurl.indexOf('www.')==0  ) baseurl.replace('www.', '')
  // console.log("newBaseUrl",baseurl);
  const url = new URL(
...
/**
 * Fetch an image and return its ArrayBuffer
 */
async function getImageBuffer(src) {
  ...
  console.log("fetechedbuffer", buffer);
  return buffer;
}
...
function base64ToArrayBuffer(base64) {
  const binaryString = atob(base64.split(",")[1]); // Remove data:image/... part
...
/**
 * Main function to check if the fetched image is the default one
 * @param {string} fetchedUrl - URL of the image from the service
 * @param {string} defaultBase64 - Base64 string of the default image
 * @returns {Promise<boolean>}
 */
async function isDefaultIcon(fetchedUrl, defaultBase64) {
  try {
    const [fetchedBuffer, defaultBuffer] = await Promise.all([
      getImageBuffer(fetchedUrl),
      base64ToArrayBuffer(defaultBase64),
    ]);
    return compareBuffers(fetchedBuffer, defaultBuffer);
  } catch (err) {
